...
class EditFormEstadoFitosanitarioScreen extends StatefulWidget {
  const EditFormEstadoFitosanitarioScreen({Key? key}) : super(key: key);

  @override
  _EditFormEstadoFitosanitarioScreen createState() =>
      _EditFormEstadoFitosanitarioScreen();
}

class _EditFormEstadoFitosanitarioScreen
    extends State<EditFormEstadoFitosanitarioScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    EstadoFitosanitario? objArgumento;
    objArgumento =
        ModalRoute.of(context)!.settings.arguments as EstadoFitosanitario?;

    if (_boolInicio) {
      getTextDescricion.text = objArgumento!.getDescripcion_ef!;
      _boolInicio = false;
    }

    int? idPe = objArgumento!.getId_ef;
    return WillPopScope(
          ...
          actions: <Widget>[
            IconButton(
              onPressed: () {
                try {
                  setState(() async {
                    EstadoFitosanitarioDB objBD = EstadoFitosanitarioDB();
                    String accion = await objBD.Eliminar_EF(idPe!);
                ...
                children: <Widget>[
                  ...
                  Container(
                    ...
                    child: MaterialButton(
                      onPressed: () {
                        try {
                          if (globalFormkey_EstadoFitosanitario.currentState!
                              .validate()) {
                            if (getTextDescricion.text.length > 3) {
                              setState(() async {
                                EstadoFitosanitario obj = EstadoFitosanitario(
                                ...
                                String accion = await objBD.Editar_EF(obj);
                                if (accion == "ACCIÓN CORRECTA") {
                                  Navigator.pop(context);
                                  Navigator.of(context).pushReplacementNamed(
                                      '/list_form_e_f_screen');
