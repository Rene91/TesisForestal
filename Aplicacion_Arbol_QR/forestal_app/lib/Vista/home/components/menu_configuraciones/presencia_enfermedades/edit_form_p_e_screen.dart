...
class EditFormPresenciaEnfermedadesScreen extends StatefulWidget {
  const EditFormPresenciaEnfermedadesScreen({Key? key}) : super(key: key);

  @override
  _EditFormPresenciaEnfermedadesScreen createState() =>
      _EditFormPresenciaEnfermedadesScreen();
}

class _EditFormPresenciaEnfermedadesScreen
    extends State<EditFormPresenciaEnfermedadesScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    PresenciaEnfermedades? objArgumento;
    objArgumento =
        ModalRoute.of(context)!.settings.arguments as PresenciaEnfermedades?;

    if (_boolInicio) {
      getTextDescricion.text = objArgumento!.getDescripcion_pe!;
      _boolInicio = false;
    }
    int idPe = objArgumento!.getId_pe!;
    return WillPopScope(
          ...
          actions: <Widget>[
            IconButton(
              onPressed: () {
                try {
                  setState(() async {
                    PresenciaEnfermedadesDB objBD = PresenciaEnfermedadesDB();
                    String accion = await objBD.Eliminar_PE(idPe);
                          ...
                          if (globalFormkey_PresenciaEnfermedades.currentState!
                            ...
                            if (getTextDescricion.text.length > 3) {
                              setState(() async {
                                ...
                                String accion = await objBD.Editar_PE(obj);
